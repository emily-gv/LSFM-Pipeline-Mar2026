#from tensorflow.keras.models import load_model, Model # load from a folder as the model to make it work in newer TF Keras
...
def prediction(model, image_path, norm_type=None):
    ...
    preds_test = model(img, training=False) #, verbose=0)#model.predict(img, verbose=0)
    preds_test = tf.argmax(preds_test, axis=-1) * 50
...
def segment_folder(model, frame_path, output_folder):
    ...
    for img in imgs:
        img_name = img[img.rfind(os.sep) + 1:]#this gets the image name out of the path
        pred = prediction(model, img, norm_type)
        cv2.imwrite(os.path.join(output_folder, img_name), pred)
    return 
...
def assess_models(models_dir, frame_path, mask_path, outputs_dir):
    ...
    for file in os.listdir(models_dir):
        if file.endswith('.h5'):
            ...
            model_subdir = outputs_dir + file[:-3] + os.sep
            try:
...
def assess_models_in_folders(top_dir, frame_path, mask_path, outputs_dir):
    ...
    for file in os.listdir(top_dir):
        ...
        if os.path.isdir(path):
            assess_models(path + os.sep, frame_path, mask_path, outputs_dir)
